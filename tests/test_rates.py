...
from kimmdy_hydrolysis.rates import e_ts1, e_ts2, low_force_log_rate, high_force_log_rate, experimental_reaction_rate_per_s, theoretical_reaction_rate_per_s

def test_ets1():
    assert e_ts1(0,0,0) == 0.
    assert e_ts1(1,1,1) == 0.
    assert e_ts1() == 80.

def test_ets2():
    assert e_ts2(0,0,0) == 0.
    assert e_ts2(1,1,1) == 0.
    assert e_ts2() == 92.
